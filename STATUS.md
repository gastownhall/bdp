...
| --- | --- | --- |
| Read profile | yes | sealed two-target evidence cohort — packaged, self-certified in-process, and honestly not-applicable capability-gated rows recorded per target in the artifact, which carries the authoritative counts — historical window verified in CI; this draft changes Read schema/manifest and awaits prerequisite integration and a genuine successor cohort ([correction](docs/design/read-erasure-pointer-correction.md)) |
...
evidence:verify`. Its current historical-window pass does not establish coverage
for the changed Read inputs described in the correction. Runner reports keep
`claimEligible` false by construction.
