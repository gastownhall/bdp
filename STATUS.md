...
| Read+Update profile | draft — sequence request/response envelopes, mutation results, singleton request records, the two alias targets (`put-alias` and `delete-alias`, with their alias result, sequence membership, and the alias-path uniqueness invariant), idempotency-key syntax and qualification, duplicate handling, dependency normalization, replay re-authorization, outcome retention, durability and recovery, and eleven problem rows drafted in the specification and schema bundle with illustrative fixtures and 66 unclaimed catalog rows; review council 9 folded (decisions D21–D31 and revisions); on 2026-09-08 D29 ruled C and applied, cross-packet X1 ruled B and applied as D32 (D9 superseded), and D31 ruled B — alias mutation joins the profile — and applied as D33–D37; the remaining provisional decisions in `docs/design/w1-read-update-decisions.md` pend the operator's rulings | not yet realized — no manifest, fixture realization, runner, or evidence exists for any Read+Update row |
