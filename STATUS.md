...
| Transactional profile (batch, receipts, Events, snapshots, changefeed) | draft — applied 2026-09-08 from `docs/design/w1-transactional-packet.md` after decisions T1–T48 and X1–X4 were ruled or ratified: the owned-Link delta, batch envelopes and the endpoint/status matrix, Mutation Receipts (five closed representations) and the Transactional problem table (three rows; direct and receipt contexts closed), transaction-level idempotency and `sequence` on a Transactional Scope, and version erasure on the changefeed with the erasure ledger are drafted in the specification and schema bundle (55 Transactional definitions; 138 in all, the sealed Read definitions byte-identical) with thirteen illustrative fixture files (one separate 68-case HTTP-contract family with 22 schema-validated response bodies and eleven exchange fixtures, 83 schema-validated exchange bodies, one intentionally malformed parsed request and two raw I-JSON request texts and four separately validated committed-group examples, plus eight `sha-256-jcs` digest vectors) and 133 unclaimed catalog rows, twelve of which retire the Read+Update rows the profile contradicts through the catalog's new `retires` member; T47 ruled (b) — a failed receipt is retained for at least `retention.receipt`, then forgotten whole, never expired; T49 option 1 (locator-only alias receipts), T63(a) (withheld allocation projection), and T64(a) (caught-up live erasure publication) are ACKed and materialized 2026-09-08; T62 option 1 and its two direct-carrier retry choices (T62a/b = A) are ruled and materialized 2026-09-09: bounded unresolved comparison and same-epoch fresh admission after retraction, with immutable attempt ownership, one overall comparison deadline, and the existing nonwaiting sequence precedence; T65 = A is ruled and materialized for persistent Event consumers claiming protocol-backed erasure handling through existing changefeed/snapshot-ledger recovery and retained-copy cleanup, preserving every-store duties and keeping History H12 separate; T50–T56 (including amended T56) and T58–T61 recommendation A are ratified, preserving their already applied choices; T57 option B selects actual canonicalization regression checks using #24's existing serializer after authorized integration, with independent expected vectors and current checks preserved until then — that follow-up is not implemented here; RP1 is handled separately (see the packet's dated ACK records); these ACKs grant no readiness or merge permission. Known gaps of the draft: no RFC 8785 serializer exists in the repository, so the digest vectors are reproduced from their recorded serializations and the canonicalization is not recomputed (T57); the I-JSON string and duplicate-member rules were restored in council 13 independently of #23’s number model (T56 amended); council 13 amends snapshot projection, the status matrix, owned-Link erasure deltas and admission wording under existing rulings; G1–G5 = A are ACKed and materialized 2026-09-09: same-manifest GET/HEAD, same-epoch issued receipt-page survival, shared bodyless pre-admission 406, omitted receipt/finite-feed validators with HTTP conditionals and native bodyless 304/412, and fixed HEAD parity; shared Read implementation and successor catalog/manifest/evidence adoption remain separate; the packet's other section 7 residuals (the synchronous wait bound, minimum-position wait, Event correlation across sources, the administrative erasure surface, owned Links and Selectors, cross-implementation rows, `bdptest` controls, per-Resource set diagnostics, discovery `ETag` semantics, and the ledger's size) stand | not yet realized — no manifest, fixture realization, runner, or evidence exists for any Transactional row |
...
G1–G5 evidence boundary (2026-09-09): the shared 406/conditional HTTP rules
are newly specified. The historical Read cohort is preserved and does not
attest to them. The separate HTTP fixture family checks narrated examples
and corruption rejection only; no new runtime behavior or grant is claimed.
The genuine successor Read integration must adopt applicable catalog and
manifest assertions and collect observations before a new claim.
